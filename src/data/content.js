...
(function () {
  ...
  const BUILDINGS = {
    // —— 采矿（3）——
    mining_machine: { id: 'mining_machine', name: '采矿机', shortName: '采矿机', icon: '⛏️', color: '#c49654', kind: 'miner', family: null, tier: 1, w: 80, h: 80, speed: 1, powerDemandKw: 420, powerGenerationKw: 0, inputCapacity: 0, outputCapacity: 180, techId: null, mineKind: 'solid', description: '安放在矿脉上持续开采矿石，是最早的自动化劳力。' },
    oil_extractor: { id: 'oil_extractor', name: '原油萃取站', shortName: '萃取站', icon: '🛢️', color: '#c09559', kind: 'miner', family: null, tier: 2, w: 80, h: 80, speed: 1, powerDemandKw: 840, powerGenerationKw: 0, inputCapacity: 0, outputCapacity: 300, techId: 'fluid_handling', mineItems: ['crude_oil'], description: '立在油泉之上把原油源源不断地抽上地面。' },
    water_pump: { id: 'water_pump', name: '抽水站', shortName: '抽水站', icon: '💧', color: '#599bc0', kind: 'miner', family: null, tier: 1, w: 80, h: 80, speed: 1, powerDemandKw: 300, powerGenerationKw: 0, inputCapacity: 0, outputCapacity: 300, techId: 'fluid_handling', mineItems: ['water', 'sulfuric_acid', 'hydrogen'], description: '部署在水源、酸湖或氢气泉上抽取水、酸液与氢气，化工产线的起点。' },
    advanced_miner: { id: 'advanced_miner', name: '深层采矿机', shortName: '深采机', icon: '🦾', color: '#cd9d4c', kind: 'miner', family: null, tier: 2, w: 80, h: 80, speed: 2, powerDemandKw: 1050, powerGenerationKw: 0, inputCapacity: 0, outputCapacity: 360, techId: 'advanced_mining', mineKind: 'solid', description: '以两倍速率深挖矿脉核心，中后期采矿的主力。' },
  ...
  const TECHNOLOGIES = {
    ...
    // 红线：semiconductor_process / processor_tech / planetary_logistics 一律不得消耗结构矩阵。
    // 否则「跨星物流 ← 结构矩阵 ← 钛晶石 ← 有机晶体（另一颗星球）」重新闭环，
    // 玩家在没有物流时永远拿不到结构矩阵，进度死锁（2026-09-10 事故根因）。
    semiconductor_process: { id: 'semiconductor_process', name: '半导体工艺', tier: 4, costs: [{ itemId: 'electromagnetic_matrix', amount: 16 }, { itemId: 'energy_matrix', amount: 16 }], prerequisites: ['high_strength_crystal'], summary: '从硅锭到晶圆，踏入微观制造的大门。', unlocks: ['硅晶圆', '微晶元件'] },
    processor_tech: { id: 'processor_tech', name: '处理器架构', tier: 5, costs: [{ itemId: 'electromagnetic_matrix', amount: 18 }, { itemId: 'energy_matrix', amount: 18 }], prerequisites: ['semiconductor_process'], summary: '把百万晶体管集成进一颗控制核心。', unlocks: ['处理器'] },
    planetary_logistics: { id: 'planetary_logistics', name: '行星物流', tier: 6, costs: [{ itemId: 'electromagnetic_matrix', amount: 16 }, { itemId: 'energy_matrix', amount: 16 }], prerequisites: ['motor_drive', 'processor_tech'], summary: '让运输机与货运飞船取代长长的传送带走廊。', unlocks: ['行星物流站', '物流运输机', '跨行星航线'] },
    ...
    interstellar_logistics: { id: 'interstellar_logistics', name: '星际物流', tier: 7, costs: [{ itemId: 'electromagnetic_matrix', amount: 20 }, { itemId: 'energy_matrix', amount: 20 }], prerequisites: ['planetary_logistics', 'energy_storage'], summary: '跨行星航线与轨道采集网络由此展开：启明 I 的水油石与焰砾 II 的硅钛从此对流。', unlocks: ['星际物流站', '物流运输船', '轨道采集器', '跨行星运输调度'] },
  ...
  const PLANETS = {
    ...
    // 与启明 I 严格互补：焰砾 II 有硅/钛/硫酸/金伯利却没有原油（有机晶体必须从母星运来），
    // 启明 I 有水/原油/石/煤/铁/铜却没有硅/钛（处理器与钛材必须从焰砾 II 运来）。
    // 跨星物流因此成为中期核心玩法，回归见 tests/planet-complementarity.test.mjs。
    yanli: { id: 'yanli', name: '焰砾 II', code: '熔岩星', color: '#d8794d', environment: '熔岩行星', systemId: 'dawnlight', orbitIndex: 2, solarMultiplier: 1.4, isHome: false, oreTypes: ['iron_ore', 'copper_ore', 'stone', 'silicon_ore', 'titanium_ore', 'coal', 'water', 'hydrogen', 'sulfuric_acid', 'kimberlite_ore'], description: '地表是奔流的熔岩，裂缝里喷出氢气、渗出硫酸，岩层中埋着钛与硅；地下水足以驱动化工，却唯独挖不出原油——有机晶体得从启明 I 运来。' },
